...
const raw = fs.readFileSync('C:/Users/21109/Desktop/# 感觉器官.txt', 'utf-8');
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (ansSection === 'fillAns') {
    const m = line.match(/^(\d+)\.\s*(.+)/);
    if (m) answers.fill[parseInt(m[1])] = parseFillAnswer(m[2]);
  }
...
// ── Build pool ──
const pool = { chapterId: 'chapter-07', questions: [] };
let mc = 0, dx = 0, mj = 0;
...
fillBlanks.forEach((q) => {
  ...
  pool.questions.push({
    ...
    blanks, answer: ans.join('、'), explanation: '',
  });
});
...
const poolPath = path.join(__dirname, '..', 'content', 'chapter-07-感觉器', 'practice-pool.json');
...
console.log(`Total: ${pool.questions.length} → ${poolPath}`);
